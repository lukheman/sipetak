                        ...
                        <div class="row">
                            ...
                            <div class="col-md-7">
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea wire:model="form.deskripsi" class="form-control" id="deskripsi" rows="5"
                            ...
                            <div class="col-md-5">
                                <label class="form-label fw-bold">Hama / Penyakit</label>

                                @php
                                    $hamaList = $this->allPenyebabSerangan->where('tipe', 'hama');
                                    $penyakitList = $this->allPenyebabSerangan->where('tipe', 'penyakit');
                                @endphp

                                <div class="border rounded p-3" style="max-height: 380px; overflow-y: auto;">
                                    @if ($hamaList->count() > 0)
                                        <p class="mb-1 text-muted small fw-semibold"><i class="mdi mdi-bug"></i> Hama</p>
                                        <div class="ps-1 mb-2">
                                            @foreach ($hamaList as $hama)
                                                <div class="form-check">
                                                    <input class="form-check-input" type="checkbox"
                                                        wire:model="form.penyebab_serangan_ids"
                                                        value="{{ $hama->id }}"
                                                        id="ps_{{ $hama->id }}"
                                                        @if ($currentState === \App\Enums\State::SHOW) disabled @endif>
                                                    <label class="form-check-label" for="ps_{{ $hama->id }}">{{ $hama->nama }}</label>
                                                </div>
                                            @endforeach
                                        </div>
                                    @endif
                                    ...
                                    @if ($penyakitList->count() > 0)
                                        @if ($hamaList->count() > 0)
                                            <hr class="my-2">
                                        @endif
                                        <p class="mb-1 text-muted small fw-semibold"><i class="mdi mdi-virus"></i> Penyakit</p>
                                        <div class="ps-1">
                                            @foreach ($penyakitList as $penyakit)
                                                <div class="form-check">
                                                    <input class="form-check-input" type="checkbox"
                                                        ...
                                                        id="ps_{{ $penyakit->id }}"
                                                        @if ($currentState === \App\Enums\State::SHOW) disabled @endif>
                                                    <label class="form-check-label" for="ps_{{ $penyakit->id }}">{{ $penyakit->nama }}</label>
                                                </div>
                                            @endforeach
                                        </div>
                                    @endif

                                    @if ($hamaList->count() === 0 && $penyakitList->count() === 0)
                                        <p class="text-muted small mb-0">Belum ada data hama/penyakit.</p>
                                    @endif
                                </div>
                            </div>
                        </div>
                    ...
                    <tbody>
                        @forelse ($this->tanaman as $item)
                            <tr>
                                ...
                                <td>
                                    @if ($item->gambar)
                                        <img src="{{ asset('storage/' . $item->gambar) }}" alt="Gambar Tanaman"
                                ...
                                <td>
                                    @forelse ($item->penyebabSerangan as $ps)
                                        <span
                                            class="badge {{ $ps->tipe === 'hama' ? 'bg-warning' : 'bg-danger' }} mb-1">{{ $ps->nama }}</span>
                                    @empty
                                ...
                                <td class="text-end">
                                    ...
                                    @else
                                        <a href="{{ route('print-laporan.hasil-panen', ['idTanaman' => $item->id]) }}"
